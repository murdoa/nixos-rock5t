...
let
  ...
  pv = pkgs.pv;
  img = "${nixosConfig.config.system.build.sdImage}/sd-image/*.img";
in
rec {
  ...
  flash-sd = pkgs.writeShellScriptBin "flash-sd" ''
    ...
    ${pv}/bin/pv ${img} | sudo dd of=$DEVICE_PATH bs=4M oflag=sync

    printf "U-Boot successfully written to $DEVICE_PATH.\n"
  '';

  flash-all = pkgs.writeShellScriptBin "flash-all" ''
    #!/usr/bin/env bash
    set -e
    printf "Flashing U-Boot to Rock 5T...\nEnsure command is run with permissions to access rk board.\n\n"
    ...
    printf "\nUploading U-Boot Image...\n"
    ${rkdeveloptool}/bin/rkdeveloptool wl 0 ${u-boot}/u-boot-rockchip-spi.bin
    printf "U-Boot successfully flashed to Rock 5T.\n"

    printf "\nFlashing SD on Rock 5T.\n"
    ${rkdeveloptool}/bin/rkdeveloptool cs 2
    ${rkdeveloptool}/bin/rkdeveloptool wl 0 ${img}
    printf "NixOS image successfully flashed to Rock 5T.\n"
